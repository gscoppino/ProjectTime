...
class ProjectTable(tables.Table):
    name = tables.Column(
        linkify=lambda record: reverse('project:project-update', args=(record.pk,))
    )
...
class ChargeTable(tables.Table):
    project = tables.Column(
        linkify=lambda record: reverse('project:charge-update', args=(record.pk,))
    )
    ...
    class Meta:
        model = Charge
        fields = ('project', 'start_time', 'end_time', 'db_time_charged', 'closed',)
        attrs = {'class': 'table stack hover'}
